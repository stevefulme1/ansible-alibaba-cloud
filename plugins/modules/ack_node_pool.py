#!/usr/bin/python
# Copyright: (c) 2026, Steve Fulmer (@stevefulme1)
# GNU General Public License v3.0+
# (see COPYING or https://www.gnu.org/licenses/gpl-3.0.txt)

"""Ansible module: stevefulme1.alibaba_cloud.ack_node_pool"""

from __future__ import absolute_import, division, print_function

__metaclass__ = type


DOCUMENTATION = r"""
---
module: ack_node_pool
short_description: Manage ACK node pools.
description:
  - Create, update, or delete node pools in an Alibaba Cloud ACK cluster.
  - Supports check mode and is idempotent.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
extends_documentation_fragment:
  - stevefulme1.alibaba_cloud.alibaba_cloud
options:
  state:
    description: Desired state of the resource.
    type: str
    choices: ['present', 'absent']
    default: present
  cluster_id:
    description: ACK cluster ID.
    type: str
  nodepool_id:
    description: Node pool ID.
    type: str
  name:
    description: Node pool name.
    type: str
  instance_types:
    description: Instance type list.
    type: list
  desired_size:
    description: Desired number of nodes.
    type: int
  vswitch_ids:
    description: VSwitch IDs for the node pool.
    type: list
"""

EXAMPLES = r"""
- name: Manage ACK node pools
  stevefulme1.alibaba_cloud.ack_node_pool:
    access_key_id: "{{ ak }}"
    access_key_secret: "{{ sk }}"
    region_id: cn-hangzhou
    state: present
"""

RETURN = r"""
ack_node_pool:
  description: Resource details.
  returned: success
  type: dict
"""

from ansible.module_utils.basic import AnsibleModule
from ansible_collections.stevefulme1.alibaba_cloud.plugins.module_utils.alibaba_cloud import (
    AlibabaCloudClient,
    AlibabaCloudError,
    alibaba_argument_spec,
)


def main():
    spec = dict(
        state=dict(type="str", choices=["present", "absent"], default="present"),
        cluster_id=dict(type="str"),
        nodepool_id=dict(type="str"),
        name=dict(type="str"),
        instance_types=dict(type="list"),
        desired_size=dict(type="int"),
        vswitch_ids=dict(type="list"),
    )
    spec.update(alibaba_argument_spec)

    module = AnsibleModule(
        argument_spec=spec,
        supports_check_mode=True,
    )

    client = AlibabaCloudClient(
        access_key_id=module.params["access_key_id"],
        access_key_secret=module.params["access_key_secret"],
        region_id=module.params["region_id"],
        security_token=module.params.get("security_token"),
        timeout=module.params["timeout"],
    )

    state = module.params["state"]
    changed = False

    try:
        # Describe existing resources to check idempotency.
        existing = client.get(
            "DescribeClusterNodePools",
            {},
            service_endpoint="cs.aliyuncs.com",
            api_version="2015-12-15",
        )

        data = existing
        for key in "nodepools".split("."):
            data = data.get(key, {})
        if not isinstance(data, list):
            data = []

        if state == "present":
            if not data:
                if module.check_mode:
                    module.exit_json(changed=True)
                result = client.get(
                    "CreateClusterNodePool",
                    {},
                    service_endpoint="cs.aliyuncs.com",
                    api_version="2015-12-15",
                )
                changed = True
                module.exit_json(
                    changed=changed,
                    ack_node_pool=result,
                )
            else:
                module.exit_json(
                    changed=False,
                    ack_node_pool=data[0],
                )

        else:
            if data:
                if module.check_mode:
                    module.exit_json(changed=True)
                client.get(
                    "DeleteClusterNodepool",
                    {},
                    service_endpoint="cs.aliyuncs.com",
                    api_version="2015-12-15",
                )
                changed = True
            module.exit_json(changed=changed)

    except AlibabaCloudError as exc:
        module.fail_json(msg=str(exc))


if __name__ == "__main__":
    main()
