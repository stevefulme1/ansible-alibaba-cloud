"""Unit tests for stevefulme1.alibaba_cloud.vpc module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.alibaba_cloud.plugins.modules.vpc"
CLIENT_PATH = "ansible_collections.stevefulme1.alibaba_cloud.plugins.module_utils.alibaba_cloud"


@pytest.fixture
def mock_api_client():
    """Mock API client for vpc."""
    client = MagicMock()
...
def existing_resource():
    """Return a dict representing an existing vpc."""
    return {
        "vpc_id": "res-123",
        "vpc_name": "test-vpc",
        "state": "active",
...
class TestCreateVpc:
    """Tests for creating a vpc."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("vpc", {"vpc_name": "test-vpc"})
        assert result["vpc_id"] == "res-123"
        assert result["vpc_name"] == "test-vpc"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("vpc", params)
        mock_api_client.create.assert_called_once_with("vpc", params)
...
class TestUpdateVpc:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("vpc", "res-123", {"vpc_name": "updated"})
        assert result["vpc_name"] == "test-vpc-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"vpc_name": existing_resource["vpc_name"]}
        current = {"vpc_name": existing_resource["vpc_name"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("vpc", "bad-id", {})


class TestDeleteVpc:
    """Tests for deleting a vpc."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("vpc", "res-123")
        mock_api_client.delete.assert_called_once_with("vpc", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("vpc", "missing-id")
...
class TestGetVpc:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("vpc", "res-123")


class TestListVpc:
    """Tests for listing vpc resources."""
...
class TestIdempotencyVpc:
    """Tests for idempotent behavior of vpc."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("vpc", "res-123")
        desired_params = {"vpc_name": current["vpc_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["vpc_name"] != current["vpc_name"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("vpc", "missing") is not None
        assert exists is False


class TestErrorHandlingVpc:
    """Tests for error handling in vpc."""
