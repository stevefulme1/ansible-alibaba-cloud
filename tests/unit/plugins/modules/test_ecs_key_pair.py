"""Unit tests for stevefulme1.alibaba_cloud.ecs_key_pair module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.alibaba_cloud.plugins.modules.ecs_key_pair"
...
class TestCreateEcsKeyPair:
    """Tests for creating a ecs_key_pair."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("ecs_key_pair", {"key_pair_name": "test-ecs_key_pair"})
        assert result["key_pair_name"] == "res-123"
        assert result["key_pair_name"] == "test-ecs_key_pair"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "key_pair_name": "full-ecs_key_pair",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("ecs_key_pair", params)
        mock_api_client.create.assert_called_once_with("ecs_key_pair", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("ecs_key_pair", {"key_pair_name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "ecs_key_pair": {}}
        else:
            result = mock_api_client.create("ecs_key_pair", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateEcsKeyPair:
    """Tests for updating a ecs_key_pair."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("ecs_key_pair", "res-123", {"key_pair_name": "updated"})
        assert result["key_pair_name"] == "test-ecs_key_pair-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"key_pair_name": existing_resource["key_pair_name"]}
        current = {"key_pair_name": existing_resource["key_pair_name"]}
    ...
    def test_update_detects_changes(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        desired = {"key_pair_name": "new-name"}
        current = {"key_pair_name": existing_resource["key_pair_name"]}
        changed = desired != current
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("ecs_key_pair", "bad-id", {})


class TestDeleteEcsKeyPair:
    """Tests for deleting a ecs_key_pair."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("ecs_key_pair", "res-123")
        mock_api_client.delete.assert_called_once_with("ecs_key_pair", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("ecs_key_pair", "missing-id")
...
class TestGetEcsKeyPair:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("ecs_key_pair", "res-123")


class TestListEcsKeyPair:
    """Tests for listing ecs_key_pair resources."""
...
class TestIdempotencyEcsKeyPair:
    """Tests for idempotent behavior of ecs_key_pair."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("ecs_key_pair", "res-123")
        desired_params = {"key_pair_name": current["key_pair_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["key_pair_name"] != current["key_pair_name"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("ecs_key_pair", "missing") is not None
        assert exists is False


class TestErrorHandlingEcsKeyPair:
    """Tests for error handling in ecs_key_pair."""
