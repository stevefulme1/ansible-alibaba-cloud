"""Unit tests for stevefulme1.alibaba_cloud.oss_bucket module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.alibaba_cloud.plugins.modules.oss_bucket"
...
class TestCreateOssBucket:
    """Tests for creating a oss_bucket."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("oss_bucket", {"bucket_name": "test-oss_bucket"})
        assert result["bucket_name"] == "res-123"
        assert result["bucket_name"] == "test-oss_bucket"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "bucket_name": "full-oss_bucket",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("oss_bucket", params)
        mock_api_client.create.assert_called_once_with("oss_bucket", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("oss_bucket", {"bucket_name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "oss_bucket": {}}
        else:
            result = mock_api_client.create("oss_bucket", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateOssBucket:
    """Tests for updating a oss_bucket."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("oss_bucket", "res-123", {"bucket_name": "updated"})
        assert result["bucket_name"] == "test-oss_bucket-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"bucket_name": existing_resource["bucket_name"]}
        current = {"bucket_name": existing_resource["bucket_name"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("oss_bucket", "bad-id", {})


class TestDeleteOssBucket:
    """Tests for deleting a oss_bucket."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("oss_bucket", "res-123")
        mock_api_client.delete.assert_called_once_with("oss_bucket", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("oss_bucket", "missing-id")
...
class TestGetOssBucket:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("oss_bucket", "res-123")


class TestListOssBucket:
    """Tests for listing oss_bucket resources."""
...
class TestIdempotencyOssBucket:
    """Tests for idempotent behavior of oss_bucket."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("oss_bucket", "res-123")
        desired_params = {"bucket_name": current["bucket_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["bucket_name"] != current["bucket_name"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("oss_bucket", "missing") is not None
        assert exists is False


class TestErrorHandlingOssBucket:
    """Tests for error handling in oss_bucket."""
