"""Unit tests for stevefulme1.alibaba_cloud.nat_gateway module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.alibaba_cloud.plugins.modules.nat_gateway"
CLIENT_PATH = "ansible_collections.stevefulme1.alibaba_cloud.plugins.module_utils.alibaba_cloud"


@pytest.fixture
def mock_api_client():
    """Mock API client for nat_gateway."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"nat_gateway_id": "res-123", "name": "test-nat_gateway"}
    client.update.return_value = {"nat_gateway_id": "res-123", "new_name": "test-nat_gateway-updated"}
...
def existing_resource():
    """Return a dict representing an existing nat_gateway."""
    return {
        "nat_gateway_id": "res-123",
        "name": "test-nat_gateway",
        "state": "active",
    }


class TestCreateNatGateway:
    """Tests for creating a nat_gateway."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("nat_gateway", {"name": "test-nat_gateway"})
        assert result["nat_gateway_id"] == "res-123"
        assert result["name"] == "test-nat_gateway"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "name": "full-nat_gateway",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("nat_gateway", params)
        mock_api_client.create.assert_called_once_with("nat_gateway", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("nat_gateway", {"name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "nat_gateway": {}}
        else:
            result = mock_api_client.create("nat_gateway", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateNatGateway:
    """Tests for updating a nat_gateway."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("nat_gateway", "res-123", {"name": "updated"})
        assert result["name"] == "test-nat_gateway-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("nat_gateway", "bad-id", {})


class TestDeleteNatGateway:
    """Tests for deleting a nat_gateway."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("nat_gateway", "res-123")
        mock_api_client.delete.assert_called_once_with("nat_gateway", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("nat_gateway", "missing-id")
...
class TestGetNatGateway:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("nat_gateway", "res-123")


class TestListNatGateway:
    """Tests for listing nat_gateway resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"nat_gateway_id": "1", "name": "first"},
            {"nat_gateway_id": "2", "new_name": "second"},
        ]
        result = mock_api_client.list("nat_gateway")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"nat_gateway_id": "1", "name": "match"}]
        result = mock_api_client.list("nat_gateway", filters={"new_name": "match"})
        assert len(result) == 1


class TestIdempotencyNatGateway:
    """Tests for idempotent behavior of nat_gateway."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("nat_gateway", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("nat_gateway", "missing") is not None
        assert exists is False


class TestErrorHandlingNatGateway:
    """Tests for error handling in nat_gateway."""
